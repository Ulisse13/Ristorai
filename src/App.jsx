...
function Invoices({ invs, setInvs, ings, setIngs, fornitori, setFornitori, learned, setLearned, isMobile, setNavBack, clearNavBack, pushHistory }) {
  ...
  async function handleFile(f) {
    ...
    try {
      ...
      let extractedText = ""
      const PROMPT = promptBase || `Sei un esperto contabile per la ristorazione. Analizza questa fattura e restituisci SOLO JSON valido senza markdown. REGOLE NOME: massimo 5 parole, solo il prodotto. NO pesi, NO volumi, NO codici articolo. ECCEZIONE: se l'unità di misura è pz, conf, cf, cassa, cartone, bottiglia → INCLUDI il peso/volume nel nome (es. "Maionese 5kg", "Uova 30pz", "Olio EVO 5l"). PRESERVARE SEMPRE nel nome queste specificazioni se presenti: Conservazione: al naturale, sott'aceto, sott'olio, affumicato, salmistrato, marinato, fermentato Stato: fresco, precotto, prec, cotto, crudo, abbattuto, abb, decongelato, dec Surgelazione: surgelato, gelo, IQF, frozen, ultra frozen, superfrozen, glassato, glass, -18 Tagli: intero, metà, filetti, filoni, trancio, pulito, sporco, piccolo, medio, grande Osso: C/O (con osso), S/O (senza osso), DIS (disossato) Testa: C/T (con testa), S/T (senza testa) Calibri gamberi: 1°, 2°, 3°, 4° Calibri polpo: T1 T2 T3 T4 T5 T6 T7 T8 T9 Calibri calamari: U5 U10 1P 2P 3P 4P Stato finale: S (surgelato), C (congelato), F (fresco) — solo se isolata come ultima parola Abbreviazioni: B.A. (bovino adulto), S/V (sottovuoto), ATM (atmosfera modificata) Esempi: "Gambero Rosso 2°", "Polpo Pulito T2", "Calamaro IQF U5", "Salmone Affumicato", "Fesa B.A. C/O", "Petto Barberie", "Peperoni Rossi al Naturale", "Coscia Pollo S", "Calamaro Sporco 2P IQF" REGOLE PREZZO: copia il valore della colonna Prezzo. Se c'è colonna Sconto: applica prezzoUnitario = Prezzo x (1 - Sconto/100). I numeri 4,5,10,22 in ultima colonna sono IVA non sconti. REGOLE UNITA: usa SEMPRE l'unità di misura dalla colonna UM/U.M. della fattura convertita in minuscolo. KG/Kg→kg, LT/Lt/LITRI→l, PZ/pz/NR/nr/N→pz, ML/ml→ml. Default kg per carni, pesce, formaggi, verdura, frutta. Default l per liquidi e bevande. Usa pz solo per prodotti venduti a pezzo intero (uova, limoni, dadi). CATEGORIE: Carni, Pesce, Frutta e Verdura, Freschi, Surgelati, Dispensa CARNI - sotto1: Bovino, Maiale, Pollo, Tacchino, Agnello, Anatra, Coniglio, Selvaggina, Avicoli PESCE - sotto1: Orata, Branzino, Salmone, Pesce Spada, Tonno, Ricciola, Dentice, Cernia, Ombrina, Crostacei, Molluschi, Scampi, Totani, Canocchie, Altri Pesci FRESCHI - sotto1: Formaggi Nobili, Latticini, Salumi, Altri Freschi FRUTTA E VERDURA - sotto1: Frutta, Verdure, Erbe aromatiche SURGELATI - sotto1: Carni, Pesce, Verdure, Gelati e Dolci, Preparati DISPENSA - sotto1: Conserve, Condimenti, Secchi, Bevande analcoliche, Bevande alcoliche, Superalcolici, Detersivi SURGELATI: prodotti con parola surgelato/frozen/ultra frozen/superfrozen/gelo/congelato/IQF/glassato/glass/abbattuto/ABB oppure lettera S o C isolata come ultima parola oppure -18. {"fornitore":"","numero":"","data":"YYYY-MM-DD","totale":0,"iva":0,"prodotti":[{"nome":"","categoria":"","sotto1":"","sotto2":"","quantita":0,"unita":"kg o pz o l","prezzoUnitario":0,"sconto":"","produttore":""}]}`
      if (isPdf) {
  ...
  function save() {
    ...
    if (toAdd.length > 0) {
      const newIngs = toAdd.map(p => ({
        ...
        cat: p.cat,
        unit: p.cat === "Vini" ? "bottiglia" : (p.confUnit || (["kg","l","litri","pz","ml","g"].includes((p.unita||"").toLowerCase()) ? (p.unita === "litri" ? "l" : p.unita.toLowerCase()) : "kg")),
